...
function Get-D365AddinFolders {
    ...
    foreach ($pf in @($env:ProgramFiles, ${env:ProgramFiles(x86)}) | Select-Object -Unique) {
...
try {
    if ($Source) {
        ...
        if ($Dev) {
            ...
            foreach ($f in $files) {
                Invoke-WebRequest "$baseUrl/$f" -OutFile (Join-Path $work $f) -UseBasicParsing
            }
        }
        else {
            ...
            $tag = (Invoke-WebRequest "https://api.github.com/repos/$repo/releases/latest" -UseBasicParsing |
                        ConvertFrom-Json).tag_name
            Write-Host "`nDownloading release $tag..." -ForegroundColor Cyan
            foreach ($f in $files) {
                Invoke-WebRequest "https://github.com/$repo/releases/download/$tag/$f" -OutFile (Join-Path $work $f) -UseBasicParsing
            }
        }
    }
