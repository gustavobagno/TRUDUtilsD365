...
namespace InstallToVS
{
    ...
    class Program
    {
        ...
        private static IEnumerable<string> FindExtensionFolders()
        {
            ...
            // 1) Explicit override wins and is trusted as-is (kept for backwards compatibility).
            string env = Environment.GetEnvironmentVariable("DynamicsVSTools");
            if (!string.IsNullOrEmpty(env))
            {
                AddFolder(result, Path.Combine(env, AddinFolder), validate: false);
            }
        ...
        private static IEnumerable<string> GetDefaultVsRoots()
        {
            ...
            foreach (string pf in programFiles.Distinct())
            {
